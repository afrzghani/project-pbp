...
class AiNoteService
{
    ...
    private function requestGroq(Note $note, string $content): ?array
    {
        ...
        if ($apiKey === '') {
            Log::warning('GROQ_API_KEY kosong. Menggunakan fallback AI lokal.', ['note_id' => $note->id]);
        ...
        try {
            ...
            $response = Http::withToken($apiKey)
                ...
                ->post($endpoint, [
                    ...
                    'messages' => [
                        [
                            'role' => 'system',
                            'content' => 'You summarize Indonesian lecture notes and create five concise flashcards. Always respond with valid JSON.',
                        ],
                        [
                            'role' => 'user',
                            'content' => $prompt,
            ...
            if (! $raw) {
                Log::warning('Groq API tidak mengembalikan konten.', ['note_id' => $note->id]);
    ...
    private function fallbackResult(string $content): array
    {
        ...
        $flashcards = $sentences
            ...
            ->map(function ($sentence, $index) {
                ...
                // Extract key concept from sentence to create a direct question
                // Remove trailing punctuation
                $cleanSentence = rtrim($sentence, '.!?');
                
                // Simple heuristic: create question from key words
                // Try to extract the main subject/concept
                $words = explode(' ', $cleanSentence);
                
                // If sentence has structure, create question about the key concept
                if (count($words) >= 3) {
                    // Take meaningful words (skip common words)
                    ...
                    if (count($meaningfulWords) > 0) {
                        // Take first 3-5 meaningful words as question topic
                    ...
                    // Short sentence, just make it a question
        ...
        if (empty($flashcards)) {
            // Create a direct question from the content
    ...
    private function decodeResponse(string $raw, int $noteId): array
    {
        try {
            ...
            Log::warning('Groq response bukan JSON valid', [
                'note_id' => $noteId,
                'raw' => $raw,
            ]);

            throw new RuntimeException('Groq mengembalikan JSON yang tidak valid.');
        }
