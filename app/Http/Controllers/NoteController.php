...
class NoteController extends Controller
{
    ...
    public function store(StoreNoteRequest $request): RedirectResponse
    {
        try {
            ...
            $note->save(); // Save first to get ID
            ...
            // Save again if needed (e.g. source_type updated in handleFileUpload)
            if ($note->isDirty()) {
                $note->save();
            }
    ...
    public function update(UpdateNoteRequest $request, Note $note): RedirectResponse
    {
        ...
        try {
            ...
            // Always update status and visibility from request
            // Check all possible sources: request input, request all, and validated data
            ...
            if (array_key_exists('content_html', $validated)) {
                ...
                // If content_html is provided, also update content_text (may be auto-generated)
                if (array_key_exists('content_text', $data)) {
                    $note->content_text = $data['content_text'];
                }
            } elseif (array_key_exists('content_text', $validated)) {
                // Only update content_text if content_html was not provided
                $note->content_text = $data['content_text'];
            }
            ...
            if ($note->status === 'published') {
                // Only set published_at if not already set (preserve original publish date)
    ...
    private function handleFileUpload(Request $request, Note $note): void
    {
        ...
        if ($request->hasFile('file')) {
            ...
            // Legacy fields for backward compatibility (optional, can be removed if fully migrated)
            $note->file_path = $path;
